 ...
 */
  ...
  processVOs(result: any): SoundResultVO[]{
    ...
    for (let providerName in soundsResultProviders){
      const providerSounds = soundsResultProviders[providerName].results;
      for (let soundId: number = 0; soundId < providerSounds.length; soundId++){
        ...
        if (previewParts && previewParts.length >= 3){
          spectrogram = previewParts[1] + 'displays' + previewParts[2] + '_spec_M.jpg';
        }
        let waveform: string = null;
        if (previewParts && previewParts.length >= 3) {
          waveform = previewParts[1] + 'displays' + previewParts[2] + '_wave_L.png';
        }
